...
'''
def read_person_mask(mask_path, clothes_mode = True):
    if not os.path.exists(mask_path):
        return None
    with Image.open(mask_path) as mask_img:
        mask_array = np.array(mask_img,dtype=np.uint8)
    if clothes_mode: # return (np.array, shape = [3, height, weight], this array is 0 or 1 array)
    ...
    else: # return (np.array, shape = [height,weight])
        mask_out = mask_array
    return mask_out
