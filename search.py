...
def extract_feat(model, dataloader, output_dir):
    ...
    for img_batch, cid, img_name_batch in tqdm(dataloader):
        ...
        cid = cid.cuda()
        with torch.no_grad():
            feat_batch = model(img_batch, cid)
...
def search_persons(query_name_l, feats, name_l, threshold = 1, topk = 100):
    ...
    dist_m = compute_dis_matrix(feats_query, feats_gallery, 'euclidean', is_re_ranking=True)
...
if __name__ == '__main__':
    ...
    output_feat_dir = './output/feat'
    dir_path = '../dataset/sjtu'
    ...
    # person search
    result = search_persons(query_name_l, feats, name_l, topk=300)
    write_to_excel(result,'./result.xlsx')

    # save to
    print('Save Result')
    for key, values in result.items():
        output_dir = os.path.join('output/search/', key)
