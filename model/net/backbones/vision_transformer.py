# --------------------------------------------------------
# Copy vision_transformer From SimMIM (https://github.com/microsoft/SimMIM)
# --------------------------------------------------------
...
class VisionTransformerForJoint(nn.Module):
    """ Vision Transformer Backbone For JointFormer
    """
    def __init__(self, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, init_values=None,
                 use_abs_pos_emb=True, use_rel_pos_bias=False,
                 use_mean_pooling=True, init_scale=0.001, num_patches = 18):
        super().__init__()
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim
        ...
        # for occluded joint feature patch
    ...
    def forward_features(self, x, vis_score):
        # x shape : B, P, Dim
        # vis_score: B, P
        B, P, _ = x.size()
        # append mask token to unseen patch
        mask_token = self.mask_token.expand(B, P, -1)
        w = vis_score.flatten(1).unsqueeze(-1).type_as(mask_token)
