...
class JointFromer(nn.Module):
    ...
    def __init__(self, num_classes, parts = 18, in_planes = 768):
        ...
        # extract feature
        self.feature_map_extract = nn.Conv2d(3, 768, 26, (10, 6))
        # patch embeding
        #self.proj = torch.nn.AdaptiveAvgPool2d((1,1))
        self.proj = torch.nn.AdaptiveMaxPool2d((1,1))
        # vit backbone network
        self.transformer = TransReID(num_classes=num_classes, num_patches=parts,embed_dim=self.in_planes,depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, drop_rate = 0.25)
